...
#include <linux/i2c.h>
#include <linux/i2c/atmel_mxt_ts_fe582.h>
...
#define MXT_PROCI_PRTRANSMISSIONCOMPENSATION_T80 80
...
#define MXT_T80_DISABLE 	0

/* MXT T80 Configuration */
#define MXT_T80_CTRL_ENABLE  (1 << 0)
...
#define TOUCH_ENABLE2_GPIO	159
#endif
#define TOUCH_ID_GPIO		45
#define TOUCH_INT_GPIO		50
#define TOUCH_STYLUS_GPIO	10
#define TOUCH_GLOVE_GPIO	63
#define DISPLAY_ID_GPIO		158
#define TOUCH_RST_GPIO		191
#define TPID1_GPIO		161
#define TPID2_GPIO		160
#define STYLUS_COUNT	2500

#define DRIVER_VERSION	"0.3.4"
#define MXT_CONFIG_NAME_1666T2_ER "mTX1666T2_cfg_ER.raw"
...
static	u8 current_batlvl = 3;
static	u16 batlvl_count = 0;
static	u8 active_rep = 8;
static	u8 idle_rep = 25;
...
//extern bool get_hall_trigger(viod);
...
static int mxt_t70_configuration(struct mxt_data *data, u16 cmd_offset, u8 status);
static int mxt_t79_configuration(struct mxt_data *data, u16 cmd_offset, u8 status);
static int mxt_t80_configuration(struct mxt_data *data, u16 cmd_offset, u8 status);

static void mxt_init_gpio_poweron()
{
	int err = 0;
	
	if( gpio_request(TOUCH_RST_GPIO, "AtmelTouch-reset") != 0)
	{
		printk("[Atmel]: reset gpio %d request fail.\n",TOUCH_RST_GPIO);
	}
	err = gpio_direction_output(TOUCH_RST_GPIO, 0);	//reset set low
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set TOUCH_RST_GPIO interrupt direction, error=%d.\n", err);
	}
	msleep(100);
	
	#ifdef TP_ENABLE
	printk("[Atmel]: TP_Enable gpio: %d  TP_Enable2 gpio: %d.\n",TOUCH_ENABLE_GPIO,TOUCH_ENABLE2_GPIO);
	if( gpio_request(TOUCH_ENABLE2_GPIO, "AtmelTouch-Enable2") != 0)
	{
		printk("[Atmel]: Enable gpio %d request fail.\n",TOUCH_ENABLE2_GPIO);
	}
	err = gpio_direction_output(TOUCH_ENABLE2_GPIO, 1);	//Enable2 set high
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set TOUCH_ENABLE_GPIO interrupt direction, error=%d.\n", err);
	}
	msleep(10);
	
	
	if( gpio_request(TOUCH_ENABLE_GPIO, "AtmelTouch-Enable") != 0)
	{
		printk("[Atmel]: Enable gpio %d request fail.\n",TOUCH_ENABLE_GPIO);
	}
	err = gpio_direction_output(TOUCH_ENABLE_GPIO, 1);	//Enable set high
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set TOUCH_ENABLE_GPIO interrupt direction, error=%d.\n", err);
	}
	...
	err = gpio_direction_output(TOUCH_RST_GPIO, 1);	//reset set low
	msleep(50);
}

static void mxt_gpio_poweron(struct mxt_data *data)
{
	int err = 0;
...
static void mxt_gpio_poweroff()
{
	...
	err = gpio_direction_output(TOUCH_ENABLE_GPIO, 0);	//Enable set high
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set TOUCH_ENABLE_GPIO interrupt direction, error=%d.\n", err);
	}
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if(retry_count < 3){
			dev_err(&client->dev, "%s: i2c transfer failed (%d) ,retry_count:%d\n",
				__func__, ret, retry_count);
			retry_count++;
			msleep(50 * retry_count);
			goto retry_read;
		} else {
...
static void mxt_proc_t93_messages(struct mxt_data *data, u8 *msg)
{
	...
	printk("[Atmel] %s:T93 double tap %d\n", __func__,status);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (id < 0)
		return;
	if(data->z_stylus == 0){
		cancel_delayed_work(&data->power_save_work);
	}
	if(power_save_mode == true){
		mxt_set_t7_power_cfg(touch_chip, MXT_POWER_CFG_RUN);
		//mxt_t80_configuration(touch_chip, 0, MXT_T80_ENABLE);
	}
	...
	if(active){	
		...
		if(type == MXT_T100_TYPE_ACTIVE_STYLUS && 
			...
			if(data->debug_log_enabled == 2){
				printk("[Atmel]eraser= %d, barrel= %d\n",eraser,barrel);
			}
		}
			
		if(data->debug_log_enabled == 3){
			printk("[Atmel] mesg[6]=0x%2x, tool=%d, batlvl=%d, type=%d\n",message[6],tool,batlvl,type);
		}
		if(data->debug_log_enabled == 2 || data->debug_log_enabled == 3){
			printk("[Atmel]id= %d, x= %d, y= %d, major= %d ",id,x,y,major);
		...
		input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, 0);
		if(data->z_stylus == 0){
			queue_delayed_work(maxtouch_wq, &data->power_save_work, 50*HZ);
		}
		if(data->debug_log_enabled == 2 || data->debug_log_enabled == 3){
			printk("[Atmel] Touch up!!!!\n");
		}
...
static int mxt_t70_configuration(struct mxt_data *data, u16 cmd_offset,
				u8 status)
{
	u16 reg;
    u8 command_register;
	...
	//printk("[Atmel] mxt_t70_configuration\n");
	
    reg = data->T70_address + cmd_offset;
...
static int mxt_t79_configuration(struct mxt_data *data, u16 cmd_offset,
				u8 status)
{
	u16 reg;
    u8 command_register;
	...
	//printk("[Atmel] mxt_t79_configuration\n");
	
    reg = data->T79_address + cmd_offset;
...
static int mxt_t80_configuration(struct mxt_data *data, u16 cmd_offset,
				u8 status)
{
	u16 reg;
    u8 command_register;
	...
	return 0;
	//printk("[Atmel] mxt_t80_configuration\n");
	...
	if(status == MXT_T80_DISABLE)
		ret = mxt_write_reg(data->client, reg, 0);
	else	
		ret = mxt_write_reg(data->client, reg, 1);
			       ...
			       unsigned int data_pos, int config_match)
{
	...
	while (data_pos < cfg->size) {
		...
		if (ret == 0) {
			...
			dev_err(dev, "Bad format: failed to parse object Retry Again, data_pos:%d, cfg->size:%d\n",data_pos,cfg->size);
			ret = sscanf(cfg->data + data_pos, "%x %x %x%n",
			     &type, &instance, &size, &offset);
			...
			if (ret == 0) {
				...
				dev_err(dev, "Bad format: failed to parse object, data_pos:%d, cfg->size:%d\n",data_pos,cfg->size);
				return -EINVAL;
			}
		}
		data_pos += offset;
	...
	if((config_MSB > data->maj_version) || (config_LSB > data->min_version)){
		dev_err(dev, "File config version(%d-%d) is large than chip config version(%d-%d)\n",
	...
	if((config_MSB == data->maj_version) && (config_LSB == data->min_version) && config_match == 0){
		dev_err(dev, "File config version(%d-%d) and chip config version(%d-%d) is the same, but checksum is difficult\n",
				config_MSB, config_LSB, data->maj_version,data->min_version);
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg, int config_check)
{
	...
	if(config_check == 1){
		...
		if (!ret){
			printk("[Atmel] No Need to update Config\n");
			goto release_mem;
		}else{
			printk("[Atmel] Need to update Config\n");
		}
	}
	...
	while(data->config_crc != config_crc || retry == 0){	
		...
		if (ret)
			goto release_mem;
		if(retry == 0)
			printk("[Atmel] Config CRC 0x%06X, retry:%d\n",data->config_crc, retry);	
		retry++;
		if(retry > 4){
			ret = -1;
			break;
	...
	if(ret == 0){
		...
		printk("[Atmel] Config Version : %d.%d\n",data->maj_version,data->min_version);
	} else{
		dev_err(dev, "Config updated fail ret:%d\n",ret);
	}
				  ...
				  struct mxt_object *object_table)
{
	...
	for (i = 0; i < data->info->object_num; i++) {
		...
		case MXT_SPT_TOUCHEVENTTRIGGER_T79:
			data->T79_address = object->start_address;
			break;
		case MXT_PROCI_PRTRANSMISSIONCOMPENSATION_T80:
				 ...
				 const struct firmware *cfg, int config_check)
{
	...
	while(retry < 3){	
		if (cfg) {
			...
			if (error2)
				dev_warn(dev, "error2 %d updating config\n", error2);
		} else {
			error2 = 0;
		}
		...
		if(error2 == 0){
			...
			retry ++;
			msleep(500);
			dev_err(dev, "mxt_configure_objects retry %d!!!\n",retry);
		}
	...
	if(retry == 3 && error2 != 0){
		dev_err(dev, "mxt_configure_objects retry fail!!!\n");
		return error2;
	}
	return 0;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	printk("[Ateml]: %u.%u.%02X-%u.%u-%06x\n",
			 data->info->version >> 4, data->info->version & 0xf,
			 data->info->build,data->maj_version,data->min_version,touch_chip->config_crc);
...
static int mxt_load_cfg(struct device *dev)
{
	...
	if(Read_HW_ID() == 4){
		error = request_firmware(&cfg, MXT_CONFIG_NAME_1666T2_ER, dev);
	} else {
		error = request_firmware(&cfg, MXT_CONFIG_NAME_1666T2, dev);
	}
	if (error < 0) {
		if(Read_HW_ID() == 4){
			dev_err(dev, "Failure to request config file %s\n",MXT_CONFIG_NAME_1666T2_ER);
		} else {	
...
static int mxt_load_fw(struct device *dev,int reset,int path)
{
	...
	while (pos < fw->size) {
		...
		if (frame % 50 == 0)
			dev_err(dev, "Sent %d frames, %d/%zd bytes\n",
				frame, pos, fw->size);
	}
...
static int mxt_self_update_firmware(struct mxt_data *data){
	...
	if(data->info != NULL){
		...
		dev_err(dev, "The data->info is NULL\n");
	}
	
	printk("[Atmel] Image firmware version:%d.%d, Chip firmware version:%d.%d. \n",
		NEWEST_FW_VERSION,NEWEST_FW_BUILD,chip_fw_version,chip_fw_build);
		
	/*if((NEWEST_FW_VERSION > data->info->version) || 
	 ((NEWEST_FW_VERSION == data->info->version) && (NEWEST_FW_BUILD > data->info->build))){*/
	 
	if((NEWEST_FW_VERSION != chip_fw_version) || (NEWEST_FW_BUILD != chip_fw_build)){
	
		printk("[Atmel] Need to update firmware ++ \n");
		error = mxt_load_fw(dev, 1, 1);
		if(error){
			dev_err(dev, "The self update firmware update failed(%d), retry again\n", error);
			error = mxt_load_fw(dev, 1, 1);
		}
		
		if(error){
			dev_err(dev, "The self update firmware force update failed(%d)\n", error);
...
static int mxt_self_update_cfg(struct mxt_data *data, int config_check){
	...
	if(Read_HW_ID() == 4){
		ret = request_firmware(&cfg, MXT_CONFIG_NAME_1666T2_ER, dev);
	} else {
...
static void mxt_power_save(struct work_struct *work)
{
	...
	if(build_version != 1){	
		...
		//mxt_t80_configuration(touch_chip, 0, MXT_T80_DISABLE);
	}
...
static void mxt_update_firmware_cfg(struct work_struct *work)
{
	...
	if(build_version != 1){
		...
		if(touch_chip->maj_version == 0 && touch_chip->min_version == 0 || ret != 0){
			printk("[Atmel] %s ++ retry update config\n",__func__);
			mxt_gpio_poweroff();
			mdelay(100);
			mxt_gpio_poweron(touch_chip);
			mdelay(2500);
			mxt_initialize(touch_chip);
			mdelay(150);
			ret = mxt_self_update_cfg(touch_chip, 0);
		}
	}
	read_update_result();
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '0'){
		printk(KERN_INFO "[Ateml]: Close debug log\n");
		data->debug_log_enabled = 0;
	}
	else if (buf[0] == '2'){
		printk(KERN_INFO "[Ateml]: Open debug log status: 2\n");
		data->debug_log_enabled = 2;
	}else if (buf[0] == '3'){
		printk(KERN_INFO "[Ateml]: Open debug log status: 3\n");
...
//For Stylus data services++++++
static int read_update_result(void){
	char file_path[] = "/data/data/stylus_result";
	...
	if(!IS_ERR_OR_NULL(filePtr)) {
		...
		printk(KERN_ERR "[Atmel] %s: read %d - %d. \n", __func__, touch_chip->stylus_status.attach, touch_chip->stylus_status.active);
		return 0;
	}else if(PTR_ERR(filePtr) == -ENOENT) {
		touch_chip->stylus_status.attach = 0;
		touch_chip->stylus_status.active = 0;
		printk(KERN_ERR "[Atmel] %s: %s not found\n", __func__, file_path);
		return 1;
	} else {
		touch_chip->stylus_status.attach = 0;
		touch_chip->stylus_status.active = 0;
		printk(KERN_ERR "[Atmel] %s: %s open error\n", __func__, file_path);
		return 1;
	}
	return 1;
}
...
static int update_result_to_file(int status){
	...
	if(!IS_ERR_OR_NULL(filePtr)) {
		...
		set_fs(oldfs);
		filp_close(filePtr, NULL);
		printk("[Atmel] %s: write %s done. \n", __func__, file_path);
		return 0;
	}else if(PTR_ERR(filePtr) == -ENOENT) {
		printk("[Atmel] %s: %s not found\n", __func__, file_path);
		return 1;
	} else {
		printk("[Atmel] %s: %s open error\n", __func__, file_path);
		return 1;
	}
	return 1;
}
...
{
	printk("[Ateml]: Disable_irq(%d).\n",touch_chip->irq);
	disable_irq(touch_chip->irq);
	return 0;
}

static ssize_t atmel_get_display_id(struct device *dev,struct device_attribute *attr, char *buf)
{
	printk("[Ateml]: display_id(%d).\n",gpio_get_value(DISPLAY_ID_GPIO));
	return sprintf(buf, "%d \n",(gpio_get_value(DISPLAY_ID_GPIO)==0) ? 0:1);
}
//			display_id/touch_ic_id/tp_id1/tp_id2
//			AUO/GIS-A	Tianma/GIS-A	
// Atmel	1(0/0/0/0)	2(0/1/0/0)	
//
//					AUO/GIS-B	Tianma/GIS-B	AUO/toptouch	Tianma/toptouch
// Goodix(black)	3(1/0/0/0)	4(1/0/0/0)		5(1/0/0/0)		6(1/0/1/0)
// Goodix(white)	7(1/0/1/0)	8(1/0/1/0)		9(1/0/1/0)		10(1/0/1/0)

static ssize_t atmel_get_tp_sku(struct device *dev,struct device_attribute *attr, char *buf)
{
	int display_id,touch_ic_id,tp_id1,tp_id2,result=0;

	display_id=(gpio_get_value(DISPLAY_ID_GPIO)==0) ? 0:1;
	touch_ic_id=(gpio_get_value(TOUCH_ID_GPIO)==0) ? 0:1;
	tp_id1=(gpio_get_value(TPID1_GPIO)==0)?0:1; //color
	tp_id2=(gpio_get_value(TPID2_GPIO)==0)?0:1; //vendor
	...
	printk("[Ateml]: display_id(%d). touch_ic_id(%d). tp_id1(%d). tp_id2(%d). result(%d)\n",
	display_id,touch_ic_id,tp_id1,tp_id2,result);
	return sprintf(buf, "%d \n",result);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[Ateml]: active_rep(%d). idle_rep(%d)\n",active_rep, idle_rep);
	return count;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[Ateml]: gesture_active_rep(%d). gesture_idle_rep(%d)\n",gesture_active_rep, gesture_idle_rep);
	return count;
}

static atmel_update_firmware_config(struct device *dev,struct device_attribute *attr, char *buf)
{	
	int err = 0,retry = 0;
	struct input_dev *input_dev = touch_chip->input_dev;
	...
	if(touch_chip->update_result == 0){
		...
		{
			err = mxt_self_update_cfg(touch_chip, 0);
			printk("[Atmel] mxt_self_update_cfg fail, retry:%d\n",retry);
			retry++;
			mdelay(500);
		}
...
static DEVICE_ATTR(enable_gpio, S_IRUGO, atmel_enable_gpio, NULL);
static DEVICE_ATTR(disable_gpio, S_IRUGO, atmel_disable_gpio, NULL);
static DEVICE_ATTR(display_id, S_IRUGO, atmel_get_display_id, NULL);
static DEVICE_ATTR(update_touch_firmware_config, S_IRUGO, atmel_update_firmware_config, NULL);
static DEVICE_ATTR(tp_sku, S_IRUGO, atmel_get_tp_sku, NULL);
...
static struct attribute *mxt_attrs[] = {
	...
	&dev_attr_disable_gpio.attr,
	&dev_attr_display_id.attr,
	&dev_attr_tp_sku.attr,
...
#ifdef mxt_double_tap
static ssize_t mxt_proc_double_tap_read(struct seq_file *buf, void *v)
{
	printk("%s: double_tap_ENABLE = %d\n", touch_chip->double_tap_enable);
	return 0;
}

static ssize_t mxt_proc_double_tap_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
	if (buf[0]=='0'){
	...
	printk("%s: double_tap_ENABLE = %d\n", __func__,touch_chip->double_tap_enable);
	return len;
}
...
{	
	...
	{
		//printk("%s:: proc double_tap file create sucessed\n", __func__);
	}
	else
	{
		printk("%s:[%d]: proc double_tap file create failed\n", __func__, __LINE__);
	}
	return;
}
...
void mxt_remove_proc_double_tap_file(void)
{
	...
	printk("%s:[%d]: proc double_tap file removed.", __func__, __LINE__);
...
#ifdef mxt_gesture
static ssize_t mxt_proc_gesture_read(struct seq_file *buf, void *v)
{
	printk("%s: gesture_enable = %d\n", touch_chip->gesture_enable);
	printk("%s: gesture_type_w = %d\n", touch_chip->gesture_type_1);
	printk("%s: gesture_type_s = %d\n", touch_chip->gesture_type_2);
	printk("%s: gesture_type_e = %d\n", touch_chip->gesture_type_3);
	printk("%s: gesture_type_c = %d\n", touch_chip->gesture_type_4);
	printk("%s: gesture_type_z = %d\n", touch_chip->gesture_type_5);
	printk("%s: gesture_type_v = %d\n", touch_chip->gesture_type_6);
	return 0;
}

static ssize_t mxt_proc_gesture_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
	printk("%s: gesture_ENABLE0 = %c\n", __func__, buf[0]);
	if (buf[0]=='0'){
...
void mxt_create_proc_gesture_file(void)
{
	...
	{
		//printk("%s: proc gesture file create sucessed\n", __func__);
	}
	else
	{
...
{
	extern struct proc_dir_entry proc_root;
	printk("%s:[%d]: proc gesture file removed.", __func__, __LINE__);
...
#ifdef mxt_stylus
static ssize_t mxt_proc_stylus_read(struct seq_file *buf, void *v)
{
	return 0;
}

static ssize_t mxt_proc_stylus_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
	if (buf[0]=='0'){
...
// add by leo for testtest ++
static ssize_t mxt_proc_logtool_read(struct seq_file *buf, void *v)
{
	printk(KERN_INFO "[Ateml]: data->debug_log_enabled = %d\n", touch_chip->debug_log_enabled);
	return 0;
}

static ssize_t mxt_proc_logtool_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
	u8 i;
	...
	if (kstrtou8(buf, 0, &i) == 0 && i < 2) {
		...
		printk(KERN_INFO "[Ateml]: %s\n", i ? "debug enabled" : "debug disabled");
	} else {
		printk(KERN_INFO "[Ateml]: debug_enabled write error\n");
	}
	*/
	return len;
}
...
void mxt_create_proc_logtool_file(void)
{
	...
	if(mxt_proc_stylus_file)
	{
		//printk("%s:: proc logtool file create sucessed\n", __func__);
	}
	else
	{
		printk("%s:[%d]: proc logtool file create failed\n", __func__, __LINE__);
	}
	return;
}
...
{
	extern struct proc_dir_entry proc_root;
	printk("%s:[%d]: proc logtool file removed.", __func__, __LINE__);
	remove_proc_entry(MXT_PROC_LOGTOOL_FILE, &proc_root);
	return;
}
// add by leo for testtest --

int hall_status_notify(bool hall_trigger_suspend){
	
	if(touch_ic_status == true){
		printk(KERN_ERR "[Atmel]%s %d,%d\n", __func__,touch_chip->suspended,hall_trigger_suspend);
...
static void mxt_stop(struct mxt_data *data)
{
	...
	if (data->use_regulator){
		...
		//test for 
		//mxt_t70_configuration(data, 40 , 0);
		/*mxt_t70_configuration(data, 80 , 0);
		mxt_t79_configuration(data, 4 , 1);
		mxt_set_t8_acquisition_cfg(data, 4, 10);
		mxt_set_t8_acquisition_cfg(data, 6, 0);
		mxt_set_t8_acquisition_cfg(data, 7, 65);
		mxt_set_t8_acquisition_cfg(data, 8, 25);
		mxt_set_t8_acquisition_cfg(data, 9, 176);*/
...
static int mxt_touch_sysfs_init(void)
{
	...
	{
		printk(KERN_ERR "[Ateml]: subsystem_register failed\n");
		ret = -ENOMEM;
		return ret;
	}
	...
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file touch status failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_touch_id.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file touch id failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_display_id.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file display id failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_tp_sku.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file tp sku failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_set_report_rate.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file set report rate failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_set_gesture_report_rate.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file set gesture report rate failed\n");
		return ret;
	}
	...
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_ssn_info failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_stylus_type.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_stylus_type failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_model_name.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_model_name failed\n");
		return ret;
	}
		ret = sysfs_create_file(android_touch_kobj, &dev_attr_attached.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_attached failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_active_flag.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_active_flag rate failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_update_success.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_update_success rate failed\n");
		return ret;
	}
}
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[Atmel] %s. ver:%s. HW_ID:%d.\n",__func__,DRIVER_VERSION,Read_HW_ID());
	...
	//add by josh +++
	data->int_gpio = TOUCH_INT_GPIO;
	data->reset_gpio = TOUCH_RST_GPIO;
	...
	queue_delayed_work(data->mxt_wq, &data->touch_chip_firmware_upgrade_work, 10*HZ);
	...
	{
		printk("[Atmel]: interrupt gpio %d request fail.\n",data->int_gpio);
	}
	err = gpio_direction_input(data->int_gpio);
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set interrupt direction, error=%d.\n", err);
	}
	else
	{
		//printk("[Atmel] %s: init interrupt pin OK. \n",__func__);
	}
	
	if( gpio_request(TOUCH_GLOVE_GPIO, "TOUCH_GLOVE_GPIO") != 0 )
	{
		printk("[Atmel]: interrupt gpio %d request fail.\n",TOUCH_GLOVE_GPIO);
	}
	err = gpio_direction_input(TOUCH_GLOVE_GPIO);
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set interrupt direction, error=%d.\n", err);
	}
	...
	if( gpio_request(TOUCH_STYLUS_GPIO, "TOUCH_STYLUS_GPIO") != 0 )
	{
		printk("[Atmel]: interrupt gpio %d request fail.\n",TOUCH_STYLUS_GPIO);
	}
	err = gpio_direction_input(TOUCH_STYLUS_GPIO);
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set interrupt direction, error=%d.\n", err);
	}
	...
	err = gpio_direction_output(data->reset_gpio, 1);	//reset set high
	if(err)
	{
		printk(KERN_ERR "[Atmel]: Failed to set interrupt direction, error=%d.\n", err);
	}
	msleep(100);*/
	//printk("[Atmel] %s: init reset pin OK. \n",__func__);
	...
	if(switch_dev_register(&data->touch_sdev) < 0){
		printk("[Atmel] %s: touch switch device register failed!\n",__func__);
	}
	
	//mxt_self_update_cfg(data);
	printk("[Atmel] %s ++ success touch_ic_status: %d\n",__func__,touch_ic_status);
	return 0;
...
//err_free_mem:
	cancel_delayed_work(&data->touch_chip_firmware_upgrade_work);
	printk("[Atmel] %s ++ fail touch_ic_status: %d\n",__func__,touch_ic_status);
	//kfree(data);
...
static int mxt_suspend(struct device *dev)
{
	...
	if(touch_ic_status == true){
		...
		if(((data->double_tap_enable == 0) && (data->gesture_enable == 0)) || (build_version == 1)){
			mxt_irq_disable(data->irq);
			mxt_reset_slots(data);
			mxt_gpio_poweroff();
			return 0;
...
static void mxt_early_suspend(struct early_suspend *es)
{
	...
	if(touch_ic_status == true){
		if(((mxt->double_tap_enable == 0) && (mxt->gesture_enable == 0)) || (build_version == 1)){
		} else {
...
static int __init mxt_init(void)
{
	...
	if( gpio_request(TOUCH_ID_GPIO, "TPIC_ID") != 0)
	{
		printk("[Atmel]: reset gpio %d request fail.\n",TOUCH_ID_GPIO);
	}
	
	if(!gpio_get_value(TOUCH_ID_GPIO)){
		printk("[Atmel] TOUCH_ID_GPIO:%d\n",gpio_get_value(TOUCH_ID_GPIO));
		printk("[Atmel] DISPLAY_ID_GPIO:%d\n",gpio_get_value(DISPLAY_ID_GPIO));
		...
		mxt_init_gpio_poweron();
	
		return i2c_add_driver(&mxt_driver);
	}else{
		printk("[Atmel] TOUCH_ID_GPIO:%d skip Atmel driver\n",gpio_get_value(TOUCH_ID_GPIO));
		return 0;
	}
}
